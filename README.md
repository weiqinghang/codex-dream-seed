...
支持 macOS、Linux 和 Windows。仓库 CI 会在三个系统和 Python 3.9–3.13 的完整声明矩阵
上运行单元测试与 bootstrap；Windows 路径、权限差异和 PowerShell 安装入口由专门测试覆盖。
...
从 `v0.3.0` 升级时，先更新到 `v0.4.0` 并运行 bootstrap，再按“Workspace 与知识结构升级”
完成只读迁移预览和 V1→V2 切换；升级不会原地改写旧 Workspace。
...
`true` 后，再追加 `--apply`，验证目标后切换默认 workspace：
...
codex-dream migrate \
  ...
  --target ~/Documents/codex-dream-workspace-v2 \
  --apply
codex-dream --workspace ~/Documents/codex-dream-workspace-v2 verify
codex-dream set-default ~/Documents/codex-dream-workspace-v2
codex-dream doctor
```

V1 的三个 JSONL 会保存在目标的
...
失败信号、当前阻碍和下一步建议。反馈总数与合格任务数分开：不适用或未满足采样条件的反馈
不会增加验证进度。WIP 策略保存在 Workspace
...
Lifecycle Board 展示从 Backlog 到 Done 的状态；它不读取或管理任何用户梦境、知识或
Workspace 内容。
