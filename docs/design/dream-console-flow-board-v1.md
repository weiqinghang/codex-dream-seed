...
| 灵感身份、状态、范围、项目、提出时间 | knowledge item `candidates[]` | 支持 |
...
有效 defer 的灵感保留在“改进追踪”中，但不是 Board 泳道；`handoff_pending` 也是接续状态，
不是泳道。Board 固定为上述五列，不恢复“做梦中”“待接续”“已暂缓”或“已结束”列。

WIP 数量必须按主卡去重。Dream 运行过程在独立“梦境”页面观察，不占改进承诺 WIP；尚未被
用户拉入试用的灵感是 Inbox，也不占 WIP。默认限制是建议值，不是强制政策；用户可以超限，
...
“验证中”拥有独立列内排序，不覆盖其他泳道的全局排序：默认按 `eligible / target` 进度比例从高
到低，也可按全部已登记反馈数从高到低。反馈总数包含不适用、未合规和未定反馈，用于判断验证
是否真正产生观察；验证进度仍只统计 `eligibility=eligible`，两者不得互相替代。不能只按正向
证据数排序，否则会系统性隐藏反证并造成确认偏误。

验证进度条同时表达样本构成：正向为绿色、负向为砖红、混合/未定为灰色，未填满部分表示距离
目标仍缺的合格任务。颜色必须配合精确数字与文字图例。负向证据是合格样本对假设或成功标准的
反证，对调整边界、结束无效方案和避免错误固化有直接价值；未按被验证方式执行的坏结果只能降低
归因强度，不能单独证明方法无效。
...
```json
{
  ...
  "evidence_summary": {"observed": 4, "eligible": 3, "compliant": 3, "positive": 2, "negative": 1, "mixed": 0, "inconclusive": 0, "excluded": 1},
...
排序固定为：可立即收尾 → 超期/阻塞 → 缺验收 → 计划调整 → 新灵感。Console 提供“回到
Codex 处理”指令，不自行执行建议。
...
3. Dream 完成后，新灵感进入“待决策”，并保留来源 Dream。
4. 用户打开 Console，先看到 WIP 总览、最老卡片和最多三个确定性建议。
5. 用户按价值/影响面、提出时间或 Dream 提及次数排序，并结合项目/范围筛选。
6. 用户打开卡片详情，查看状态时间线、证据、反例、成功标准、范围和下一步后果。
   Validation 详情先展示确定性下一步建议，再列出适用条件、预期行为、可观察信号、成功标准、
   失败信号与当前阻碍；没有反馈、没有合格任务、样本不足、执行不合规、存在反证或未定结果都
   必须明确区分。
7. 对灵感，用户选择暂缓、拒绝或进入试用；WIP 已满时，Console 先展示需要收尾或调整的
   既有卡片，并要求记录超限理由。
...
- Validation 同时返回反馈总数、合格/合规数、证据方向、列内排序指标与确定性验收建议。
...
元数据、验证列进度/反馈子排序、证据构成条、键盘可访问、窄屏列表退化。
